...
public class PicColorServiceImpl implements PicColorService {
    ...
    public JSONUtil picColor(String image) throws IOException, JSONException {
        ...
        Request request = new Request.Builder()
                .url("https://aip.baidubce.com/rest/2.0/image-process/v1/colourize?access_token=" + getAccessToken(API_KEY, SECRET_KEY, HTTP_CLIENT))
                .method("POST", body)
                .addHeader("Content-Type", "application/x-www-form-urlencoded")
                .addHeader("Accept", "application/json")
                .build();
        System.out.println("[INFO]:黑白上色开始");
        Response response = HTTP_CLIENT.newCall(request).execute();
        System.out.println("[INFO]:黑白上色结束");
